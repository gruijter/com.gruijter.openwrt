...
class OpenWRTRouter {
  ...
  async getStaticRouterInfo() {
    ...
    const cmd = [
      ...
      `echo "${SEP}ETHERWAKE"`, '(command -v ether-wake || which ether-wake || test -x /usr/bin/ether-wake || test -x /usr/sbin/ether-wake) && echo "etherwake_exec" || (opkg list-installed etherwake 2>/dev/null; apk list -I etherwake 2>/dev/null) || true',
