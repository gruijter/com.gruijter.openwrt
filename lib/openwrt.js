...
class OpenWRTRouter {
  ...
  async getStaticRouterInfo() {
    ...
    const cmd = [
      ...
      `echo "${SEP}OPKG"`, '(opkg list-installed luci-base luci-app-nlbwmon nlbwmon etherwake || apk list -I luci-base luci-app-nlbwmon nlbwmon etherwake) 2>/dev/null || true;'
      + ' test -x /usr/sbin/nlbw && echo "nlbwmon_exec" || true;'
      + ' command -v ether-wake && echo "etherwake_exec" || true',
    ...
    const isEtherWakeInstalled = luciOutput.includes('etherwake') || luciOutput.includes('etherwake_exec');
  ...
  /**
   * Blocks a device from accessing the WAN or LAN.
   * @param {string} mac - MAC address of the device.
   * @param {string} [blockType='wan'] - 'wan', 'lan', or 'all'.
   * @returns {Promise<void>}
   */
  async blockDevice(mac, blockType = 'wan') {
    if (!mac) {
    ...
    for (const dest of dests) {
      const ruleName = `block_${dest}_${macLower.replace(/:/g, '')}`;
      try {
        const existingRules = await this._execSsh(`uci show firewall | grep ${ruleName}`);
        if (existingRules) {
          // console.log(`Rule ${ruleName} for ${mac} already exists.`);
          continue;
        }
      } catch (e) {
      ...
      await this._execSsh(`uci set firewall.${ruleId}.src='lan'`);
    ...
    // console.log(`Device ${mac} has been blocked for ${blockType}.`);
  }
   ...
   * @returns {Promise<void>}
   */
  async unblockDevice(mac, blockType = 'wan') {
    if (!mac) {
      throw new Error('MAC address is required.');
    }
    const macLower = mac.toLowerCase();
    ...
    for (const dest of dests) {
      ...
      let existingRules;
      try {
        existingRules = await this._execSsh(`uci show firewall | grep ${ruleName}`);
      } catch (e) {
        // grep returns non-zero exit code if no match is found
        // console.log(`No ${dest} rule for ${mac} found.`);
        continue;
      }
      ...
      const ruleIds = existingRules.trim().split('\n').map((line) => line.split('.')[1]);
    ...
    await this._execSsh('uci commit firewall');
    await this._execSsh('/etc/init.d/firewall reload');
    // console.log(`Device ${mac} has been unblocked for ${blockType}.`);
  }
  ...
  async isDeviceBlocked(mac) {
    ...
    const macLower = mac.toLowerCase();
    const wanRuleName = `block_wan_${macLower.replace(/:/g, '')}`;
    const lanRuleName = `block_lan_${macLower.replace(/:/g, '')}`;

    let wanBlocked = false;
    try {
      const existingRules = await this._execSsh(`uci show firewall | grep ${wanRuleName}`);
      if (existingRules) {
        wanBlocked = true;
      }
    } catch (e) {
    ...
    try {
      const existingRules = await this._execSsh(`uci show firewall | grep ${lanRuleName}`);
      if (existingRules) {
        lanBlocked = true;
      }
    } catch (e) {
