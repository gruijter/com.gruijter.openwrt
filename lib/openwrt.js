...
class OpenWRTRouter {
  ...
  async getStaticRouterInfo() {
    ...
    const cmd = [
      ...
      `echo "${SEP}OPKG"`, 'test -x /usr/sbin/nlbw && echo "nlbwmon_exec" || (opkg list-installed luci-base luci-app-nlbwmon nlbwmon || apk list -I luci-base luci-app-nlbwmon nlbwmon) 2>/dev/null || true',
      `echo "${SEP}ETHERWAKE"`, '(command -v ether-wake || which ether-wake || test -x /usr/bin/ether-wake || test -x /usr/sbin/ether-wake) && echo "etherwake_exec" || (opkg list-installed etherwake || apk list -I etherwake) 2>/dev/null || true',
    ...
    const wanInterfaces = [];
    if (wan.interface) wanInterfaces.push(wan.interface);
